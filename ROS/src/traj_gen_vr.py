...
"""
    Node that generates the goal trajectory the teleoperated robot must follow
"""


# Trajectory Generator for the robot's end-effector
class TrajectoryGeneratorVr:
    # Initialise movement node functionality
    def __init__(self):
        # Initialise node
        rospy.init_node("ref_trajectory_vr")
        ...
        # Right Controller subscriber
        rospy.Subscriber(
            "/oculus_quest2/right/pose", Pose, self.right_callback
        )

        # Initialise the pose and orientation variables (set a good starting position [0.5, 0.0, 0.5] depending on Sawyer's current pose)
        self.pos_x = 0.0
        self.pos_y = 0.0
        self.pos_z = 0.0

        # Make sure to initialise the rotation magnitude to any value greater than 0
        self.ori_w = 1.0
        self.ori_x = 0.0
        self.ori_y = 0.0
        self.ori_z = 0.0
